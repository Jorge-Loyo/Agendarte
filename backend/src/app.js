...
app.post("/api/debug/create-history", debugOnly, async (req, res) => {
  try {
    const appointment = await Appointment.create({
      ...
      paymentStatus: "paid",
      notes:
        "Control rutinario. Paciente presenta buen estado general. Se recomienda continuar con medicación actual.",
      createdBy: 7,
    });
...
app.post("/api/debug/create-stats-data", debugOnly, async (req, res) => {
  try {
    ...
    res.json({
      message: "Datos de estadísticas creados",
...
app.get(
  "/api/debug/professional-appointments/:userId",
  debugOnly,
  async (req, res) => {
    try {
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      console.log(
        `🚀 Servidor Agendarte corriendo en http://localhost:${PORT}`
      );
