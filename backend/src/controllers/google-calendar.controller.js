...
const { UserPreference } = require("../models"); // Asegúrate que esta ruta sea correcta
...
const createEvent = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error creando evento en Google Calendar" });
  }
};
...
const logout = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error cerrando sesión: " + error.message });
  }
};
